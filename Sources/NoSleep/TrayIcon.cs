  ...
  class TrayIcon : ApplicationContext
  {
    ...
    private void RefreshTimer_Tick(object sender, EventArgs e)
    {
      ...
      else
      {
        ...
        if (AreAnyConfiguredAppsRunning(apps))
          desired |= EXECUTION_STATE.ES_SYSTEM_REQUIRED | EXECUTION_STATE.ES_DISPLAY_REQUIRED;
      }
    ...
    private bool AreAnyConfiguredAppsRunning(List<AppEntry> apps)
    {
      try
      {
        var pathSet = apps.Where(a => !string.IsNullOrWhiteSpace(a.ExePath))
                          .Select(a => a.ExePath)
                          .ToHashSet(StringComparer.OrdinalIgnoreCase);

        var nameSet = apps
            .Where(a => !string.IsNullOrWhiteSpace(a.ExePath))
            .Select(a => Path.GetFileNameWithoutExtension(a.ExePath))
            .Where(n => !string.IsNullOrWhiteSpace(n))
            .ToHashSet(StringComparer.OrdinalIgnoreCase);

        bool result = Process.GetProcesses().Any(p =>
        {
          try
          {
            ...
            return nameSet.Contains(p.ProcessName);
          }
          catch
          {
            return false;
    ...
    static string TryGetPath(Process p)
    {
      try
      {
